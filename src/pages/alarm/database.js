...
const Alarm = ({ isAlarmEnabled, onToggle }) => {
  ...
  const handleToggle = () => {
    ...
    if (newAlarmState) {
      ...
      let notificationMessage = `Today You have ${flightTime}!`;
  
      if (flightNos.startsWith('LEAVE')) {
        notificationMessage = `Today is a Leave day for you on ${flightDate}!`;
      }
  ...
  useEffect(() => {
    ...
    const patternStTimeDate = new Date(`${flightDate}T${flightTime}`);
console.log('patternStTimeDate:', patternStTimeDate);
    const currentTime = new Date();
  
    const timeDifference = patternStTimeDate - currentTime;
