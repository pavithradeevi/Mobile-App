...
import { getRosterDataReducer } from '../../../redux/slices/rosterSlice';
      ...
      db.transaction((tx) => {
        ...
        (tx, results) => {
          if (results.rows.length > 0) {
            const flightfrom = results.rows.raw()[0].flightFrom;
            setFlightFrom(flightfrom);
            console.log(flightfrom, "flightfrommm");
            resolve(flightfrom);
          }
        },
        (_, error) => {
    ...
    async function fetchData() {
      ...
      if (flightFrom) {
        ...
        if (matchingIndex !== -1) {
          ...
          console.log('No matching iataCode found for flightFrom:', flightFrom);
        }
      } else {
        console.log('flightFrom is undefined');
      }
    }
  
    fetchData();
  }, [iataCodeData, iataData, flightFrom]);
